define('base', ['jquery', 'bootstrap', 'layer'], function ($, boots, layer) {
	'use strict';
	...
	Base.prototype = {
		...
		scrollNav: function () {
			var $index_nav = $('.header .navbar-inverse');

			function scroll_nav() {
				if ($(window).scrollTop() > 50) {
					$index_nav.addClass('transparent');
					$('.second-top').css('background-color', 'rgba(255,255,255,.9)');
				} else {
					$index_nav.removeClass('transparent');
					$('.second-top').css('background-color', '#fff');
				}
			}
	...
	(function () {
		if (Base.prototype.cookie().get('logined')) {
			$('.login_out').on('click',function(){
				Base.prototype.fetch('/mjson/logout', {},function(){
					Base.prototype.cookie().del('logined');
					Base.prototype.cookie().del('session_id');
					Base.prototype.cookie().del('select_1');
					Base.prototype.cookie().del('select_2');
			...
			var profile = Base.prototype.fetch('/mjson/personal/info', {});
			var notify = Base.prototype.fetch('/mjson/personal/notify', {});
			$.when(profile, notify).then(function (profileRes, notifyRes) {
				var profileData = profileRes[0],notifyData = notifyRes[0];
				if(profileData,notifyData){
					Base.prototype.CurrrentUser = profileData;
					// console.log($('.dropdown-toggle .txBox').eq(0));
					$('.dropdown-toggle .txBox').get(0).outerHTML = profileData.data.employee_name;
					$('.logined').show();
					$('.hidden-xs-login').html('<a id="' + notifyData.id + '" href="#"><i class="iconfont icon-xiaoxi"></i><span class="msg-number"></span></a>')
					$('.hidden-xs-login').find('.msg-number').css({
						'margin-left': '5px',
						'color': 'rgb(232,158,32)',
						'font-weight': '500'
					}).html(notifyData.data.new_message);

					var systemBtn = $('.navbar-collapse a.system_login');

					if(profileData.data.system){
						systemBtn.on('click', function(){
							location.href="http://wogaoxingjiuhao:8818/"
						})
					}else{
			...
			Base.prototype.CurrrentUser = null;
			$('.no_login').show();
		}
	})();
	...
	//
	// });
	$('#to_bbs').on('click', function(){
		location.href="http://wogaoxinjiuhao?sid="+Base.prototype.cookie().get('session_id');
		return;
	});
	return Base;
});
